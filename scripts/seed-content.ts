 ...
 * Run this after Strapi is running: npx ts-node --esm scripts/seed-content.ts
 */
...
async function seedContent(apiToken: string) {
  const headers = {
  ...
  // Seed Hero
  try {
    const heroRes = await fetch(`${STRAPI_URL}/api/hero`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ data: heroContent }),
    });
    if (heroRes.ok) {
      // Publish
      await fetch(`${STRAPI_URL}/api/hero`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ data: { ...heroContent, publishedAt: new Date().toISOString() } }),
      });
      console.log('✅ Hero content created and published');
    } else {
      console.error('❌ Hero error:', await heroRes.text());
    }
  } catch (e) {
  ...
  // Seed Overview
  try {
    const overviewRes = await fetch(`${STRAPI_URL}/api/overview`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ data: overviewContent }),
    });
    if (overviewRes.ok) {
      await fetch(`${STRAPI_URL}/api/overview`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ data: { ...overviewContent, publishedAt: new Date().toISOString() } }),
      });
      console.log('✅ Overview content created and published');
    } else {
      console.error('❌ Overview error:', await overviewRes.text());
    }
  } catch (e) {
  ...
  // Seed EOI Form
  try {
    const eoiRes = await fetch(`${STRAPI_URL}/api/eoi-form`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ data: eoiFormContent }),
    });
    if (eoiRes.ok) {
      await fetch(`${STRAPI_URL}/api/eoi-form`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ data: { ...eoiFormContent, publishedAt: new Date().toISOString() } }),
      });
      console.log('✅ EOI Form content created and published');
    } else {
      console.error('❌ EOI Form error:', await eoiRes.text());
    }
  } catch (e) {
    console.error('❌ EOI Form error:', e);
  }
...
const apiToken = process.argv[2] || process.env.STRAPI_API_TOKEN;
...
╠════════════════════════════════════════════════════════════════════╣
║                                                                    ║
║  To seed content, you need an API token with full access.          ║
║                                                                    ║
║  Steps:                                                            ║
║  1. Go to http://localhost:1337/admin                              ║
║  2. Create admin account (if first time)                           ║
║  3. Go to Settings → API Tokens → Create new API Token             ║
║  4. Name: "Seed Token", Type: "Full access"                        ║
║  5. Copy the token and run:                                        ║
║                                                                    ║
║     npx tsx scripts/seed-content.ts YOUR_API_TOKEN                 ║
║                                                                    ║
╚════════════════════════════════════════════════════════════════════╝
`);
} else {
  seedContent(apiToken);
}
